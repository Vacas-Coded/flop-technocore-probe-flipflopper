...
FAUCET_ADAPTER = pathlib.Path('/root/.hermes/document_cache/flop-technocore-probe-flipflopper/tools/activation-adapters/faucet_adapter.py')
OUT = pathlib.Path('/root/.hermes/document_cache/flop_armed_runners/claim')
...
def write_report(report: dict) -> dict[str, str]:
    ...
    jp = OUT / f'{stem}_claim_runner.json'
    mp = OUT / f'{stem}_claim_runner.md'
    jp.write_text(json.dumps(report, indent=2, ensure_ascii=False) + '\n')
    lines = [
        '# FlipFlopper Claim Runner Report', '',
...
def main():
    ap = argparse.ArgumentParser(description='Armed-but-safe claim runner')
    ap.add_argument('url')
    ...
    adapter = load_module('flip_faucet_adapter', FAUCET_ADAPTER)
    ...
    if not derived.get('ready_for_manual_claim_attempt'):
        blockers.append('not_ready_for_manual_claim_attempt')
    if not args.armed:
    ...
    report = {
        ...
        'derived': derived,
        'blockers': blockers,
        'next_steps': adapter.recommend(derived, inspection) + ['manual approval still required before any real claim flow'],
    }
